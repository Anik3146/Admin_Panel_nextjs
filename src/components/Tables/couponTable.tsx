...
const CouponTable = () => {
  ...
  const initialFormData = {
    title: 'Summer Sale 2024', // Required field (title of the coupon)
    logo: 'https://www.chanel.com/images/q_auto:good,f_auto,fl_lossy,dpr_1.1/w_386/FSH-1711464573035-m05.jpg', // Required field (coupon logo URL)
    couponCode: 'SUMMER2024', // Required field (unique coupon code)
    startTime: '2024-06-01T00:00:00Z', // Optional field (coupon start time)
    endTime: '2024-08-31T23:59:59Z', // Required field (coupon expiration time)
    discountPercentage: 15, // Required field (discount percentage)
    minimumAmount: 100, // Required field (minimum purchase amount)
    productType: 'electronics', // Required field (type of product the coupon applies to)
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/coupon`); // Replace with your API endpoint
      if (!response.ok) {
  ...
  const handleUpdate = async (e: any) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/coupon/${formDataId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleAdd = async (event: any) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/coupon/add`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user?.token}`,
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              name="startTime"
              value={
                formData.startTime ? formData.startTime.substring(0, 19) : ''
          ...
          <div>
            ...
            <input
              ...
              id="endTime"
              name="endTime"
              value={formData.endTime ? formData.endTime.substring(0, 19) : ''} // Convert to datetime-local format
              onChange={(e) =>
                setFormData({ ...formData, endTime: e.target.value })
              }
              required
