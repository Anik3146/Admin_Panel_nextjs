...
const CategoriesTable = () => {
  ...
  const initialFormData = {
    img: 'https://www.chanel.com/images/q_auto:good,f_auto,fl_lossy,dpr_1.1/w_386/FSH-1711464573035-m05.jpg', // Optional field (image URL for the category)
    parent: 'Electronics', // Required field, unique (parent category name)
    children: ['Smartphones', 'Laptops', 'Headphones'], // Array of sub-categories
    productType: 'electronics', // Required field (lowercase, product type)
    description:
      'Category for all electronic products including smartphones, laptops, and accessories.', // Optional field (description of the category)
    products: [
      '635e4c6d1f8f4b46a8a7c1e0', // Example Product ObjectId
      '635e4c6d1f8f4b46a8a7c1e1', // Another Product ObjectId
    ], // Array of ObjectIds that reference products in the "Products" collection
    status: 'Show', // Enum values: 'Show' | 'Hide'
    featured: true, // Optional field (true/false, default could be false if not provided)
    sellCount: 500, // Optional field (number of items sold, min: 0)
  };
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setFormData({ ...formData, img: uploadedImageUrl });
  ...
  const handleUpdate = async (e: any) => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/category/edit/${formDataId}`,
        {
          method: 'PATCH',
          headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/category/delete/${formDataId}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
        ...
        >
          ...
          <div>
            <label
              ...
              className="block font-medium text-gray-700 dark:text-gray-300"
            >
              Child Categories (comma separated):
            </label>
            <input
              type="text"
              id="children"
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <img src={formData.img} alt="Uploaded" className="mt-2 max-w-xs" />
          </div>
            ...
            {filteredData &&
              filteredData.map((packageItem: any, index: any) => (
                <tr key={index}>
                  ...
                  <td className="border-[#eee] px-4 py-4 dark:border-dark-3">
                    <p className="text-dark dark:text-white">
                      {/* Safely check if children is an array */}
                      {Array.isArray(packageItem?.children)
                        ? packageItem?.children.join(', ')
                  ...
                  <td className="border-[#eee] px-4 py-4 dark:border-dark-3">
                    <p className="text-dark dark:text-white">
                      {packageItem?.featured ? 'Yes' : 'No'}
                    </p>
                  </td>
                  <td className="border-[#eee] px-4 py-4 dark:border-dark-3">
                    <p className="text-dark dark:text-white">
                      {packageItem?.sellCount}
                    </p>
                  </td>
